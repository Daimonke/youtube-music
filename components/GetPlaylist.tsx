...
const GetPlaylist = ({
  ...
  const handleTab = (
    ...
    if (newAlignment === "Add" || newAlignment === "My")
  ...
  const getPlaylist = (url: string) => {
    if (!url || !url.includes("list="))
      return displayError(), setLoading(false);
    ...
    fetch(`/api/playlist/?playlistId=${playlistId}`)
      ...
      .then((res) => {
        ...
        if (playlists.some((playlist: { url: string }) => playlist.url === url))
  ...
  const handleRemove = (url: string) => {
    ...
    const newPlaylists = playlists.filter(
      (playlist: { url: string }) => playlist.url !== url
    );
                  ...
                  >
                    ...
                    <div style={{ display: "flex", gap: 15 }}>
                      <Button
                        variant="text"
                        color="error"
                        onClick={() => handleRemove(playlist.url)}
                      >
                        Remove
                      </Button>
                      <Button
                        variant="text"
                        color="primary"
                        onClick={() => getPlaylist(playlist.url)}
                      >
                        Open
                      </Button>
                    </div>
